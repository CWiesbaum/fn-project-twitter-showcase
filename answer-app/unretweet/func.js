...
fdk.handle(function (input, ctx) {
  return new Promise((resolve, reject) => {
    ...
    if (input && input.tweetID) {
      ...
      T.post('statuses/unretweet', tweet, function (err, data, response) {
        if (err) {
          reject({ "message": "Unretweet failed." });
        }
        else {
          resolve({ "message": "Unretweet succeeded!" });
        }
      });
    }
    else {
      reject({ "message": "Unretweet needs an input." });
    }
  })
})